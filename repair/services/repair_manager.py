...
class RepairManager:
    ...
    def get_repair_statistics_by_status(self):
        dashboard = []
        repairs = RepairStatus.objects.annotate(total_repairs=Count('repair'))
        for repair in list(repairs):
            dashboard.append(RepairDashboardDetail(repair.id, repair.status, repair.total_repairs).__dict__)
        return dashboard
